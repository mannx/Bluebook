...
export default function ConversionCalculatorInput(params) {
  ...
  const [inputValue, setInputValue] = React.useState(params.defaultInput || 0);
  ...
  const handleInputChange = (e) => {
    ...
    handleConversion(e.target.value, outputUnit);
  };
  const handleOutputChange = (e) => {
    setOutputUnit(e.target.value);
    handleConversion(inputUnit, e.target.value);
  };
  ...
  const button =
    params.useCallback === true ? (
      <Button onClick={callCallback}>Copy</Button>
    ) : (
      <></>
    );
...
function performConversion(inputType, inputValue, outputType) {
  ...
  for (let i = 0; i < convMappings.length; i++) {
    if (
      convMappings[i].Input == inputType &&
      convMappings[i].Output == outputType
    ) {
